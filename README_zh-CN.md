...
ClawSecbot 位于 Bot 与 LLM 服务之间，对用户输入、ToolCall、ToolCallResult、最终输出四个阶段分别检测。ShepherdGate 通过 LLM/ReActAgent 语义分析、内置安全技能和用户自定义自然语言规则，决定允许、拦截、脱敏或要求确认。仅审计模式只记录事件不阻断；大工具内容会截断并缓存，安全智能体可按需读取完整上下文。
