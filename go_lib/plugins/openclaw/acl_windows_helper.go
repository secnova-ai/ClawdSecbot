...
func isPrincipalAccessDangerous(principal, perms string) bool {
	// Broad principals are unsafe even with read-only access on sensitive paths.
	if isBroadPrincipal(principal) {
		return hasReadAccess(perms)
	}
	// Other non-whitelisted principals are treated as risky only when they can write/modify.
