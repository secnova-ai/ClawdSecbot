...
// proxy_session.go contains only Openclaw-specific proxy session functions.
...
func SyncGatewaySandboxByAssetInternal(assetID string) string {
	...
	result, err := restartOpenclawGateway(req)
	if err != nil {
...
// RestoreToInitialConfigInternal restores openclaw.json to initial config state and restarts gateway.
