...
func (bs *BatchScanSession) run(config *repository.SecurityModelConfig) {
	...
	for i, skillName := range bs.SkillOrder {
		...
		if err != nil {
			...
			if hash != "" {
				if saveErr := scanRepo.SaveSkillScanResult(&repository.SkillScanRecord{
					SkillName: skillName, SkillHash: hash, SkillPath: skillPath, SourcePlugin: nullclawPluginID, Safe: false,
		...
		if err := scanRepo.SaveSkillScanResult(&repository.SkillScanRecord{
			SkillName: skillName, SkillHash: hash, SkillPath: skillPath, SourcePlugin: nullclawPluginID, Safe: safe, Issues: issues, RiskLevel: riskLevel,
		}); err != nil {
