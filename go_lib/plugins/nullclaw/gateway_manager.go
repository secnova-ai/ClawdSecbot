...
// 防止多路径并发调用（如 ApplyNullclawConfig + SyncNullclawGateway）导致冲突。
...
func buildGatewayRestartRequestFromDB(assetID string) *GatewayRestartRequest {
	...
	config, err := repo.GetProtectionConfig(nullclawAssetName, req.AssetID)
	if err != nil {
...
// 1. 更新 config.json 配置（将 Bot 模型配置指向本地代理）
// 2. 重启 nullclaw gateway
...
func startGatewayWithProxy(proxyPort int, botModelConfig *BotModelConfig, backupDir string, assetID string) (map[string]interface{}, error) {
	...
	if IsAppStoreBuild() {
		logging.Info("[GatewayManager] Early return: App Store build, cannot update nullclaw config")
		result["error"] = "app store build cannot update nullclaw config"
		return result, nil
	}
	...
	backupPath, err := backupNullclawConfig(configPath, backupDir)
	if err != nil {
	...
	restoreOnError := func(applyErr error) (map[string]interface{}, error) {
		...
		if restoreErr := restoreNullclawConfig(configPath, backupPath); restoreErr != nil {
	...
	// Step 3: 设置 provider base_url 为本地代理地址
	if updatedProvider != nil {
		updatedProvider["base_url"] = proxyURL
	}
	logging.Info("[GatewayManager] Step 3 models.providers change base_url to local proxy address: %s", proxyURL)
	...
	logging.Info("[GatewayManager] Step 8: restarting nullclaw gateway with full config...")
	gatewayReq := buildGatewayRestartRequestFromDB(assetID)
	restartResult, restartErr := restartNullclawGateway(gatewayReq)
	if restartErr != nil {
...
func resolveNullclawBinaryPath() string {
	if p, err := exec.LookPath("nullclaw"); err == nil {
		logging.Info("[GatewayManager] resolveNullclawBinaryPath: found: %s", p)
		return p
	} else {
		logging.Warning("[GatewayManager] resolveNullclawBinaryPath: nullclaw not found in PATH: %v", err)
	}
	return ""
}
