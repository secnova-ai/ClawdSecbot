...
const nullclawLaunchAgentLabel = "com.nullclaw.daemon"
...
func restartNullclawGateway(req *GatewayRestartRequest) (map[string]interface{}, error) {
	...
	// 1) 基于资产发现/系统环境推导 nullclaw 二进制路径 + 配置路径
	...
	// 2) stop service（忽略错误，保持幂等）
	logging.Info("[GatewayManager] Step 2: Stopping service...")
	...
	unloadExistingNullclawLaunchAgent(homeDir)

	// 3) install service 并解析 plist 路径（幂等：重新生成 LaunchAgent plist）
	logging.Info("[GatewayManager] Step 3: Installing service and resolving plist path...")
...
// restartNullclawGatewaySimple 简易版网关重启：查找 nullclaw 二进制并执行 stop + start
func restartNullclawGatewaySimple() error {
	...
	logging.Info("[GatewayManager] restartNullclawGatewaySimple: stop service")
	_, _ = runNullclawGatewayCommand(binaryPath, []string{"stop"}, homeDir)

	logging.Info("[GatewayManager] restartNullclawGatewaySimple: start service")
	_, err := runNullclawGatewayCommand(binaryPath, []string{"start"}, homeDir)
	return err
}

// runNullclawGatewayCommand 通过 shell 执行 nullclaw service 命令
// macOS 需要 source shell 配置以获取正确的 PATH（nullclaw 可能通过 homebrew 等安装）
...
func installGatewayAndGetPlistPath(binaryPath string, homeDir string) (plistPath string, output string, err error) {
	if strings.TrimSpace(homeDir) != "" {
		...
		if mkErr := os.MkdirAll(launchAgentDir, 0755); mkErr != nil {
			logging.Warning("[GatewayManager] failed to create LaunchAgents dir (%s): %v", launchAgentDir, mkErr)
		}
	}
	...
	if plistPath == "" {
		plistPath = filepath.Join(homeDir, "Library", "LaunchAgents", nullclawLaunchAgentLabel+".plist")
...
func unloadExistingNullclawLaunchAgent(homeDir string) {
