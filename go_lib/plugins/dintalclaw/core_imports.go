...
// that have been migrated from dintalclaw to core packages.
...
func calculateSkillHash(skillPath string) (string, error) {
	...
	if info.IsDir() {
		return skillscan.CalculateSkillHash(skillPath)
	}
...
// toJSONString delegates to the proxy package's implementation.
