...
// handleGetProtectionPolicy handles GET /api/v1/protection/policy/{botId}
// Returns the protection policy for a specific bot.
func (s *APIServer) handleGetProtectionPolicy(w http.ResponseWriter, r *http.Request) {
	botID := r.PathValue("botId")
	if botID == "" {
		Error(w, http.StatusBadRequest, CodeInvalidParam, "botId is required")
		return
	}

	logging.Info("API: Getting protection policy for botId=%s", botID)

	repo := repository.NewProtectionRepository(nil)
	config, _, err := findProtectionConfigByBotID(repo, botID)
	if err != nil {
		logging.Error("API: Failed to query protection config: %v", err)
	...
	// Convert internal config to external API format
	response := convertToExternalPolicy(config, botID)

	userRules, _, err := repo.GetShepherdSensitiveActions(config.AssetName, botID)
	if err != nil {
...
// handleSetProtectionPolicy handles PUT /api/v1/protection/policy/{botId}
// Updates the protection policy for a specific bot.
func (s *APIServer) handleSetProtectionPolicy(w http.ResponseWriter, r *http.Request) {
	...
	logging.Info("API: Setting protection policy for botId=%s", botID)

	// Read request body
	body, err := io.ReadAll(r.Body)
	if err != nil {
		Error(w, http.StatusBadRequest, CodeInvalidParam, "failed to read request body")
		return
	}
	defer r.Body.Close()
	...
	if req.BotID != "" && req.BotID != botID {
		Error(w, http.StatusBadRequest, CodeInvalidParam, "botId in body must match URL path")
		return
	}
	...
	if err != nil {
		logging.Error("API: Failed to query existing protection config: %v", err)
		Error(w, http.StatusInternalServerError, CodeInternalError, "failed to query protection configs")
		return
	}
