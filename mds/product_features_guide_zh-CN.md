...
自动扫描本地系统，发现正在运行的 AI Bot 实例。扫描器通过采集系统快照（进程、端口、配置文件等），匹配预定义的检测规则来识别不同类型的 Bot 资产。每个发现的资产仅基于规范化后的名称和 canonical 配置路径生成唯一的 `asset_id` 指纹标识；端口和进程路径属于运行态证据，不参与 asset ID。
