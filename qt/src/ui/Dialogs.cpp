...
void OnboardingDialog::finishOnboarding() {
    ...
    const QJsonObject payload{{QStringLiteral("key"), QStringLiteral("is_first_launch")}, {QStringLiteral("value"), QStringLiteral("false")}};
    const QString json = QString::fromUtf8(QJsonDocument(payload).toJson(QJsonDocument::Compact));
    ...
    watcher->setFuture(QtConcurrent::run(bridge_->workerPool(), [bridge = bridge_, json]() {
        const QJsonObject result = bridge->call("SaveAppSettingFFI", json);
        if (result.value(QStringLiteral("success")).toBool()) {
            QString startupError;
            if (!StartupService::setEnabled(true, &startupError)) {
                AppLogger::error(QStringLiteral("Failed to enable launch at startup after onboarding: %1").arg(startupError));
            }
        }
        return result;
    }));
